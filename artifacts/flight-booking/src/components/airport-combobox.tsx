      ...
      {showDropdown && (
        <div className="absolute z-50 w-full mt-1 bg-popover border border-popover-border rounded-md shadow-lg overflow-hidden">
          {airports.length === 0 && !isFetching && (
            <div className="px-4 py-3 text-sm text-muted-foreground">No airports found</div>
          )}
          {airports.map((airport, i) => (
            <button
              ...
              type="button"
              className={cn(
                "w-full text-left px-4 py-2.5 text-sm flex items-start gap-3 transition-colors",
                i === highlighted
            ...
            >
              <span className="font-bold text-primary mt-0.5 w-10 flex-shrink-0 text-base">
                {airport.iataCode}
              </span>
